...
- The tenant navigation should expose a visible `Platform` section for users who also hold platform access, with a `Settings` shortcut that lands on `/admin` so platform-capable users do not have to discover platform administration routes manually.
- Platform Admin navigation should stay grouped by purpose: core admin pages under `Platform`, operational monitoring under `Operations` (`Jobs`, `Emails`), governance review under `Governance` (`Audit`), geography reference maintenance as its own cluster, and platform configuration / preview tools nested under `System` within the settings area rather than mixing operational pages into system settings.
